### Code regarding Tables 1, 2 and 3 of the manuscript (25 QTLs)
...
############
## we consider either 2000 SNPs, 1000 SNPs or 500 SNPs.

if (nbSNPS==2000) {
	QTLeffects[seq(1,length(QTLlocations),40)]<-0.45
} else if (nbSNPS==1000) {
	QTLeffects[seq(1,length(QTLlocations),20)]<-0.45
} else {
	## nbSNPS=500
	QTLeffects[seq(1,length(QTLlocations),10)]<-0.45	
}

### This way, when chroLength=1, QTL (with non null effects) are located every 0.04 Morgan
### when chroLength=4, QTL (with non null effects) are located every 0.16 Morgan
### when chroLength=6, QTL (with non null effects) are located every 0.24 Morgan
